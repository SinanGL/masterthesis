...
class ValueInput:
    """
    Structure the input window for AI, aystem accuracy and reliance.
...
class DisplayDetailedInformation:
    ...
    The class displays an adherence matrix and the ADAR (Ability to Discriminate between right and wrong AI advice
    gicen the observed reliance level) value.
    """
    def __init__(self, master):
        """
        Initializes frames with the given master and packs them. Assigns values to variables ca, wa, do, co and 
        creates labels with these values to put them in the adherence matrix and calculate the ADAR.
        """
        self.master = master
        ...
        self.frame_adar = tk.LabelFrame(self.master, text="ADAR")
        self.frame_adar.grid(column=0, row=2, pady=10, padx=5)
        self.frame_probability = tk.LabelFrame(self.master, text="Probabilities for randomly choosing " +
            " when to adhere to AI advice (values only valid for binary decision cases)")
        ...
        ### display adar
        adar = self.solve_adar()
        adar_text = "The ability to discriminate between right and wrong AI advice given your reliance level is (ADAR): "
        self.lbl_adar = tk.Label(self.frame_adar, text = (adar_text + str(adar)))
        self.lbl_adar.grid(column=0, columnspan=4, row=5, sticky="ew")
        ...
        point_prob_sys_accuracy_text = ("Point probability to reach observed system accuracy " +
                                  "(given the observed reliance): ")
        ...
        prob_at_above_sys_accuracy_text = ("Probability to reach or exceed the system accuracy " +
                                  "(given the observed reliance): ")
    ...
    def solve_adar(self):
        """
        Solve and return the KPI 'Ability to discriminate between right and wrong AI Advice (ADAR)'

        Returns:
        float: The ADAR value rounded to 2 decimal places.
        """
        global ai_accuracy, reliance, sys_accuracy
        if reliance == 0 or reliance ==  1: #otherwise division by zero
            ...
            # adar
            adar = round(((sys_accuracy - sys_acc_worst)/(sys_acc_best - sys_acc_worst)), 2)
        return(adar)
    ...
    def roundToNearestOdd(n):
        """
        Round the input number `n` to the nearest odd integer.

        Parameters:
        n (float): The input number to round.

        Returns:
        int: The nearest odd integer to `n`.
        """
        rounded = round(n)
        if rounded % 2 == 1:
            return rounded
    ...
    def calculate_probability(self):
        ...
        """ not necessary as num_correct_adherence does not change because it is rounded."""
        if ai_accuracy_adapt % 2 == 0:
            sys_accuracy_adapt = DisplayDetailedInformation.roundToNearestEven(sys_accuracy * 100)
        else:
            sys_accuracy_adapt = DisplayDetailedInformation.roundToNearestOdd(sys_accuracy * 100)
        
        num_correct_adherence = int((sys_accuracy_adapt+ai_accuracy_adapt+reliance_adapt-100)/2 + 0.5)

        #point probability to achieve this exact point
        x_point_lower = num_correct_adherence
        y_point_lower = reliance_adapt-num_correct_adherence
        ...
        for sys_acc_adapt_value in sys_accuracy_adapt_array: #we cannot work with num_correct_adherence here
            x_lower = int((sys_acc_adapt_value+ai_accuracy_adapt+reliance_adapt-100)/2 + 0.5)
            y_lower = reliance_adapt-int((sys_acc_adapt_value+ai_accuracy_adapt+reliance_adapt-100)/2 + 0.5)
